import React, { useEffect, useState } from 'react'
import { useDispatch } from 'react-redux'
import { toogleMenu } from '../Utils/appSlice'
import { YOUTUBE_SEARCH_API } from '../Utils/constants'
import { Link } from 'react-router-dom'

const Head = () => {

    const [search,SerSearch] = useState("")

    const [suggestion ,setSuggestions ] =useState([])
    const [showSuggestioins,SetShowSuggestions] = useState(false)




    useEffect(()=>{
        const timer =  setTimeout(() => getSearchSuggestions(),200)
        return () =>{
            clearTimeout(timer)
        }
    },[search])


    const getSearchSuggestions = async() =>{

        // console.log("API_CALL"+ search)
        const data =  await fetch(YOUTUBE_SEARCH_API+search);
        
        const json  = await data.json() 
        console.log("hello")
        
        console.log(json)
        setSuggestions(json[1])

    }

    const dispatch  = useDispatch()
    const toogleMenuHandlar =() =>{

        dispatch(toogleMenu())

    }


// console.log(showSuggestioins)
  return (
    <div className='grid grid-flow-col p-5 m-2 shadow-lg'>
        <div className='flex col-span-1 '>
        <img alt='menu' onClick={()=>toogleMenuHandlar()} className='h-8 cursor-pointer' src='https://paragondigital.com/wp-content/uploads/Menu-Icon2.jpg'/>
        <img alt='youtube' className='h-8 ml-2' src='https://upload.wikimedia.org/wikipedia/commons/thumb/b/b8/YouTube_Logo_2017.svg/1200px-YouTube_Logo_2017.svg.png'/>
        </div>
        <div className='col-span-10 px-10'>
            <div>
            <input className='w-1/2 border border-gary-400 rounded-l-full p-2'
             type='text'value={search} 
             onChange={(e)=>SerSearch(e.target.value)}
             onFocus={() =>SetShowSuggestions(true)}  
             onBlur={() =>SetShowSuggestions(false)}
             /> 
            <button className='border border-gray-400 p-2 rounded-r-full'>Search</button>
            </div>


          
            {showSuggestioins &&(
                 <div className='fixed absolute bg-white w-[37rem] shadow-lg rounded-lg py-2 px-5 border-gary-100'>
                 <ul>
                    {suggestion.map(s=> <li className='py-2 px-3 shadow-sm hover:bg-gray-100'>{s}</li>)}
                 </ul>
             </div>
            )}
           
        </div>

        <div className='col-span-1'> 
            <img  className='h-8' src ='data:image/png;base64,'/>
        </div>
    </div>
  )
}

export default Head
